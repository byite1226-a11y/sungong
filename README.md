...
> ⚠️ **이 저장소는 git clone 본에서만 작업할 것.**
> 로컬에 폴더가 두 개 있다면 — `C:\Users\testos_02\sungong` (git clone 본)과
> OneDrive 백업본(8/22자) — **OneDrive 쪽은 낡은 스냅샷입니다.** 거기서 고친 것은
> push 되지 않고, 최신 코드도 아닙니다. 항상 clone 본에서 열고 `git pull` 부터 하세요.
